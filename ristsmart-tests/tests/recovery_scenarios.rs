...
fn test_mock_recovery_functionality() {
    ...
    let mock_element = stats_mock
        .downcast_ref::<ristsmart_tests::RistStatsMock>()
        .unwrap();
    ...
    let initial_retrans_0 = initial_stats
        .get::<u64>("session-0.sent-retransmitted-packets")
        .unwrap();
    let initial_rtt_0 = initial_stats
        .get::<f64>("session-0.round-trip-time")
        .unwrap();
    let initial_original_0 = initial_stats
        .get::<u64>("session-0.sent-original-packets")
        .unwrap();

    println!(
        "Session 0 before recovery: retrans={}, rtt={:.1}ms, original={}",
    ...
    println!(
        "Stats after session 0 recovery: {}",
        recovered_stats.to_string()
    );

    let recovered_retrans_0 = recovered_stats
        .get::<u64>("session-0.sent-retransmitted-packets")
        .unwrap();
    let recovered_rtt_0 = recovered_stats
        .get::<f64>("session-0.round-trip-time")
        .unwrap();
    let recovered_original_0 = recovered_stats
        .get::<u64>("session-0.sent-original-packets")
        .unwrap();

    println!(
        "Session 0 after recovery: retrans={}, rtt={:.1}ms, original={}",
        recovered_retrans_0, recovered_rtt_0, recovered_original_0
    );

    // Verify recovery improvements
    assert!(
        recovered_retrans_0 < initial_retrans_0,
        "Recovery should reduce retransmission count"
    );
    assert!(
        recovered_rtt_0 < initial_rtt_0,
        "Recovery should improve (reduce) RTT"
    );
    assert!(
        recovered_original_0 > initial_original_0,
        "Recovery should show continued transmission progress"
    );

    // Verify session 1 was not affected by session 0 recovery
    let session1_retrans = recovered_stats
        .get::<u64>("session-1.sent-retransmitted-packets")
        .unwrap();
    let session1_rtt = recovered_stats
        .get::<f64>("session-1.round-trip-time")
        .unwrap();

    assert_eq!(
        session1_retrans, 160u64,
        "Session 1 retrans should be unchanged by session 0 recovery"
    );
    assert_eq!(
        session1_rtt, 120.0f64,
...
fn test_recovery_integration_with_dispatcher() {
    ...
    let initial_weights_json: serde_json::Value =
        serde_json::from_str(&initial_weights_str).unwrap();
    ...
    println!(
        "Initial weights with poor conditions: session-0={:.3}, session-1={:.3}",
    ...
    let recovered_weights_json: serde_json::Value =
        serde_json::from_str(&recovered_weights_str).unwrap();
    ...
    println!(
        "Weights after session 1 recovery: session-0={:.3}, session-1={:.3}",
        recovered_weight0, recovered_weight1
    );

    // Verify the dispatcher responded to the recovery
    assert!(
        recovered_weight0 > 0.0 && recovered_weight1 > 0.0,
    ...
    let session1_final_retrans = final_stats
        .get::<u64>("session-1.sent-retransmitted-packets")
        .unwrap();
...
fn test_multiple_recovery_cycles() {
    ...
    for cycle in 1..=5 {
        ...
        let current_retrans = current_stats
            .get::<u64>("session-0.sent-retransmitted-packets")
            .unwrap();
        let current_rtt = current_stats
            .get::<f64>("session-0.round-trip-time")
            .unwrap();
        let current_original = current_stats
            .get::<u64>("session-0.sent-original-packets")
            .unwrap();

        println!(
            "After cycle {}: retrans={}, rtt={:.1}ms, original={}",
            cycle, current_retrans, current_rtt, current_original
        );

        // Verify continuous improvement
        assert!(
            current_retrans <= previous_retrans,
    ...
    assert!(
        final_retrans < 250u64, // Less than half of initial 500
        "Multiple recovery cycles should significantly reduce retransmissions"
    );
    assert!(
        final_rtt < 100.0f64, // Less than half of initial 250ms
