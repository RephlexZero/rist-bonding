...
fn test_weighted_flow_distribution() {
    ...
    src_pad1.link(&counter_sink1.static_pad("sink").unwrap())
        .expect("Failed to link to counter1");
    src_pad2.link(&counter_sink2.static_pad("sink").unwrap())
        .expect("Failed to link to counter2");
    src_pad3.link(&counter_sink3.static_pad("sink").unwrap())
    ...
    println!("Buffer distribution: counter1={}, counter2={}, counter3={}", count1, count2, count3);
    println!("Total buffers: {}, Sum of counts: {}", total_buffers, count1 + count2 + count3);

    // Verify all buffers were received
    assert_eq!(count1 + count2 + count3, total_buffers as u64, 
    ...
    println!("Ratios: counter1={:.2}%, counter2={:.2}%, counter3={:.2}%", 
             ratio1 * 100.0, ratio2 * 100.0, ratio3 * 100.0);

    // Allow some tolerance for statistical variation (±5%)
    assert!((ratio1 - 0.5).abs() < 0.05, 
            "Counter1 should receive ~50% of buffers, got {:.2}%", ratio1 * 100.0);
    assert!((ratio2 - 0.3333).abs() < 0.05, 
            "Counter2 should receive ~33% of buffers, got {:.2}%", ratio2 * 100.0);
    assert!((ratio3 - 0.1667).abs() < 0.05, 
            "Counter3 should receive ~17% of buffers, got {:.2}%", ratio3 * 100.0);

    // Verify that higher weights get more buffers
    assert!(count1 > count2, "Counter1 (weight=3.0) should receive more than Counter2 (weight=2.0)");
    assert!(count2 > count3, "Counter2 (weight=2.0) should receive more than Counter3 (weight=1.0)");
...
fn test_dynamic_weight_adjustment() {
    ...
    println!("Phase 1 - Equal weights [1.0, 1.0]: counter1={}, counter2={}", 
    ...
    assert!(diff <= 20, "With equal weights, buffer counts should be similar, got difference of {}", diff);
    ...
    let weights_json: serde_json::Value = serde_json::from_str(&current_weights)
        .expect("current-weights should be valid JSON");
    let weights_array = weights_json.as_array().unwrap();
    assert_eq!(weights_array[1].as_f64().unwrap(), 5.0, "Weight 2 should be updated to 5.0");
    ...
    println!("Phase 2 - Adjusted weights [1.0, 5.0]: counter1={}, counter2={}", 
    ...
    if count2_phase2 > 0 && count1_phase2 > 0 {
        ...
        assert!(ratio >= 2.0, 
                "Counter2 should receive significantly more buffers in phase 2, ratio={:.2}", ratio);
    }
...
fn test_invalid_weights_handling() {
    ...
    let weights_json: serde_json::Value = serde_json::from_str(&weights_str)
        .expect("Should still have valid default weights");
    ...
    let weights_str: String = dispatcher.property("current-weights");
    let weights_json: serde_json::Value = serde_json::from_str(&weights_str)
        .expect("Should have valid weights");
    ...
    for (i, weight) in weights_array.iter().enumerate() {
        ...
        assert!(weight_val >= 0.0, "Weight {} should be non-negative, got {}", i, weight_val);
    }
