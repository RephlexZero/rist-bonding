...
use anyhow::Result;
use ristsmart_netem::builder::EmulatorBuilder;
...
#[allow(dead_code)]
enum Command {
    /// Start the emulator with a scenario
    Up { scenario: PathBuf },
    /// Stop the emulator
    Down { scenario: PathBuf },
    /// Run the emulator for a specified duration
    Run {
        scenario: PathBuf,
        duration: u64,
...
fn parse_args() -> Result<Command> {
    ...
    if args.len() < 3 {
        ...
        eprintln!(
            "  {} run --scenario <path> --duration <seconds> [--metrics <path>]",
            args[0]
        );
        std::process::exit(1);
    }
...
async fn main() -> Result<()> {
    ...
    match command {
        ...
        Command::Run {
            ...
            info!(
                "Running emulator for {} seconds with scenario: {}",
                duration,
                scenario.display()
            );
            ...
            if let Some(metrics_path) = metrics {
                ...
                let mut metrics_writer =
                    ristsmart_netem::metrics::MetricsWriter::new_file(&metrics_path)?;
                let mut interval = time::interval(Duration::from_secs(1));

                // Run for the specified duration, collecting metrics
                for _ in 0..duration {
                    interval.tick().await;
