use observability::{
    ObservabilityOrchestrator, MetricsCollector, QdiscParams,
    PrometheusExporter, JsonExporter, CsvExporter, MetricsExporter,
    ObservabilityConfig, TraceEntry
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("✅ Prometheus format: {} lines", prometheus_metrics.lines().count());
    ...
    // Export to files
    prometheus_exporter.export_to_file(&final_snapshot, "/tmp/rist-metrics.prom".as_ref()).await?;
    json_exporter.export_to_file(&final_snapshot, "/tmp/rist-metrics.json".as_ref()).await?;
    csv_exporter.export_to_file(&final_snapshot, "/tmp/rist-metrics.csv".as_ref()).await?;
    ...
    info!("- {} active links monitored", final_snapshot.simulation_metrics.active_links);
    info!("- {:.1} Mbps total throughput", final_snapshot.simulation_metrics.total_throughput_bps as f64 / 1_000_000.0);
    info!("- {:.1}ms average RTT", final_snapshot.simulation_metrics.avg_rtt_ms);
    info!("- {:.2}% average loss rate", final_snapshot.simulation_metrics.avg_loss_rate * 100.0);
