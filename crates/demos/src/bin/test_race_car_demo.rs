use netlink_sim::{NetworkOrchestrator, TestScenario, LinkParams};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handles = orchestrator.start_bonding_scenarios(
    ...
    for (i, handle) in handles.iter().enumerate() {
        info!("  Link {}: ingress:{} -> egress:{} (scenario: {})", 
              i + 1, handle.ingress_port, handle.egress_port, handle.scenario.name);
    }
