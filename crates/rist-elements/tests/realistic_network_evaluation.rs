...
impl RealisticNetworkProfile {
    fn new(name: &str, interface: &str, delay_ms: u32, loss_pct: f32, rate_kbps: u32, variation_secs: u64) -> Self {
        Self {
            name: name.to_string(),
...
async fn test_realistic_network_performance_1080p60_bonded_rist() {
    ...
    println!("Testing 1080p60 H.265 + AAC over 4 bonded RIST connections with real network constraints");
    ...
    for (i, profile) in profiles.iter().enumerate() {
        println!("  {}: {} - {}ms delay, {:.3}% loss, {}kbps rate", 
                i, profile.name, profile.delay_ms, 
                profile.loss_pct * 100.0, profile.rate_kbps);
    }
    ...
    for profile in &profiles {
        ...
        match apply_network_params(&qdisc_manager, &profile.interface, &params).await {
            ...
            Err(e) => {
                println!("  ⚠️ Failed to apply constraints to {} ({})", profile.interface, e);
                println!("     This is expected in CI environments without proper network capabilities");
            }
        }
    }
    ...
    let expected_weights: Vec<f64> = profiles.iter()
    ...
    for (i, (profile, weight)) in profiles.iter().zip(expected_weights.iter()).enumerate() {
        println!("  Connection {}: {}kbps capacity = {:.3} weight ({:.1}%)", 
                i, profile.rate_kbps, weight, weight * 100.0);
    }
    ...
    pipeline.add_many([&av_source, &dispatcher, &rist_stats_mock]).unwrap();
    ...
    for (i, counter) in counters.iter().enumerate() {
        ...
        let dispatcher_src = dispatcher.request_pad_simple(&format!("src_{}", i)).unwrap();
    ...
    let session_params: Arc<Mutex<HashMap<usize, NetworkParams>>> = Arc::new(Mutex::new(HashMap::new()));
    {
        let mut map = session_params.lock().unwrap();
        for (i, p) in profiles.iter().enumerate() { map.insert(i, p.to_network_params()); }
    }
    ...
    let network_update_task = tokio::spawn(async move {
        ...
        loop {
            ...
            for (idx, profile) in profiles_clone.iter_mut().enumerate() {
                ...
                let params = profile.to_network_params();
                if let Err(e) = apply_network_params(&qdisc_manager_clone, &profile.interface, &params).await {
                    // Expected to fail in CI environments
                    if tick_count <= 3 { // Only log first few failures
                        println!("  Network update for {} failed: {} (expected in CI)", profile.interface, e);
                    }
                }
                // Record last-applied params for stats feedback (even if tc apply failed in CI)
                session_params_clone.lock().unwrap().insert(idx, params.clone());
            }
    ...
    for tick in 0..test_duration_secs {
        ...
        // Calculate rates
        let rates: Vec<f64> = current_counts.iter().zip(last_counts.iter())
        ...
        let weights: Vec<f64> = if total_packets > 0 {
            current_counts.iter().map(|&count| count as f64 / total_packets as f64).collect()
        } else {
            vec![0.0; 4]
        };
    ...
    let final_weights: Vec<f64> = if total_final > 0 {
        final_counts.iter().map(|&count| count as f64 / total_final as f64).collect()
    } else {
        vec![0.0; 4]
    };
    ...
    let weight_variance = final_weights.iter()
        .zip(expected_weights.iter())
        .map(|(actual, expected)| (actual - expected).abs())
        .sum::<f64>() / 4.0;
    
    println!("Average weight deviation from expected: {:.3}", weight_variance);
...
fn test_realistic_network_performance_fallback() {
    ...
    println!("Note: Real network simulation requires root privileges and proper network capabilities.");
