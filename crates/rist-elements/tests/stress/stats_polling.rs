...
fn test_stats_driven_rebalancing() {
    ...
    assert!(
        count1 > 0 && count2 > 0,
        "Both paths should receive traffic"
    );
...
fn test_stats_structure_validation() {
    ...
    assert_eq!(
        retrieved.get::<u64>("session-0.sent-original-packets"),
        Ok(2000u64)
    );
    assert_eq!(
        retrieved.get::<u64>("session-1.sent-retransmitted-packets"),
        Ok(180u64)
    );
    assert_eq!(
        retrieved.get::<f64>("session-1.round-trip-time"),
        Ok(75.0f64)
    );
...
fn test_malformed_stats_handling() {
    ...
    assert!(
        !weights_after_empty.is_empty(),
        "Should maintain valid weights"
    );
    ...
    assert!(
        !weights_after_wrong.is_empty(),
        "Should handle wrong structure name gracefully"
    );
    ...
    assert!(
        !weights_after_invalid.is_empty(),
    ...
    for (sent, retx, rtt, description) in test_cases {
        ...
        if let Ok(quality) = get_property::<f64>(&stats_mock, "quality") {
            ...
            if retx == 0 && rtt < 50.0 {
                assert!(
                    quality > 80.0,
                    "Perfect conditions should yield high quality"
                );
            }
        }
    }
