...
mod netns_race_car_tests {
    ...
    fn build_race_car_sender_pipeline(
        bonding_addresses: &[String],
    ) -> Result<(gstreamer::Pipeline, Arc<Mutex<VideoQualityMetrics>>), Box<dyn std::error::Error>>
    {
        ...
        audiotestsrc_left
            .link(&audiomixer)
            .expect("Failed to link left channel to mixer");
        audiotestsrc_right
            .link(&audiomixer)
            .expect("Failed to link right channel to mixer");
        ...
        rtph265pay
            .link(&q_video)
            .expect("Failed to link video pay to queue");
        q_video
            .link(&rtpmux)
            .expect("Failed to link video queue to muxer");
        rtp_l16pay
            .link(&q_audio)
            .expect("Failed to link audio pay to queue");
        q_audio
            .link(&rtpmux)
            .expect("Failed to link audio queue to muxer");
    ...
    fn build_race_car_receiver_pipeline(
        listen_port: u16,
    ) -> Result<(gstreamer::Pipeline, Arc<Mutex<VideoQualityMetrics>>), Box<dyn std::error::Error>>
    {
        let pipeline = gstreamer::Pipeline::new();
        ...
        ristsrc
            .link(&rtpptdemux)
            .expect("Failed to link ristsrc to demuxer");
        ...
        // Link audio chain
        gstreamer::Element::link_many([&rtpopusdepay, &opusdec, &audioconvert, &audio_counter])
    ...
    fn build_race_car_receiver_pipeline_with_video_output(
        ...
        if let Some(vsrc) = video_parse.static_pad("src") {
            let metrics_weak: std::sync::Weak<Mutex<VideoQualityMetrics>> =
    ...
    fn build_video_only_sender_pipeline(
        bonding_addresses: &[String],
    ) -> Result<gstreamer::Pipeline, Box<dyn std::error::Error>> {
        let pipeline = gstreamer::Pipeline::new();
    ...
    fn build_ts_over_rtp_sender_pipeline(
        bonding_address: &str,
    ) -> Result<gstreamer::Pipeline, Box<dyn std::error::Error>> {
        let pipeline = gstreamer::Pipeline::new();
        ...
        let asrc_l = gstreamer::ElementFactory::make("audiotestsrc")
            .property("is-live", true)
            .property("freq", 440.0f64)
            .property("volume", 0.5f64)
            .build()?;
        let asrc_r = gstreamer::ElementFactory::make("audiotestsrc")
            .property("is-live", true)
            .property("freq", 523.0f64)
            .property("volume", 0.5f64)
            .build()?;
    ...
    fn build_ts_over_rtp_receiver_pipeline_with_output(
        ...
        tsdemux.static_pad("sink").unwrap().add_probe(
            gstreamer::PadProbeType::BUFFER,
            move |_pad, info| {
                if let Some(gstreamer::PadProbeData::Buffer(buffer)) = &info.data {
                    if let Some(pts) = buffer.pts() {
                        if pts.seconds() % 5 == 0 && pts.nseconds() < 50_000_000 {
                            // Every 5s
                            info!(
                                "🎞️ tsdemux receiving TS buffer: size={} pts={:.3}s",
                                buffer.size(),
                                pts.seconds() as f64 + pts.nseconds() as f64 / 1e9
                            );
                        }
                    }
                }
    ...
    async fn setup_race_car_network_scenarios(
        rx_port: u16,
    ) -> Result<(NetworkOrchestrator, Vec<String>), Box<dyn std::error::Error>> {
        ...
        for (i, scenario) in scenarios.into_iter().enumerate() {
            let _ = orchestrator
                .start_scenario(scenario, rx_port + i as u16)
                .await?;
        }
    ...
    async fn run_race_car_rist_test(
        duration_secs: u64,
    ) -> Result<QualityAssessment, Box<dyn std::error::Error>> {
        info!("🏎️  Starting Race Car RIST Integration Test");
        info!(
            "Duration: {}s, Video: 1080p60 H.265, Audio: Dual-Channel Two-Tone Sine Wave",
            duration_secs
        );
        ...
        // Build pipelines
        let (sender_pipeline, _sender_metrics) =
            build_race_car_sender_pipeline(&bonding_addresses)?;
        ...
        // Assess quality
        let assessment = assess_video_quality(
            &final_receiver_metrics,
            duration_secs,
            "Race Car Cellular Bonding",
        );
    ...
    async fn run_race_car_rist_test_with_video_output(
        duration_secs: u64,
    ) -> Result<QualityAssessment, Box<dyn std::error::Error>> {
        ...
        let (_orchestrator, _bonding_addresses) =
            setup_race_car_network_scenarios(rx_port_ts).await?;
        ...
        let sender_handle = tokio::task::spawn_blocking(move || {
            ...
            ns_mgr
                .attach_existing_namespace("tx0_link_1")
                .expect("Failed to attach tx0_link_1");

            let _ = ns_mgr.exec_in_namespace("tx0_link_1", || {
                // Build and run sender pipeline entirely inside tx namespace
                gstreamer::init().ok();
                let sender_pipeline =
                    build_ts_over_rtp_sender_pipeline(&ts_addr).expect("build sender");
                ...
                let _ = ctx.with_thread_default(|| {
                    ...
                    let ml_quit = main_loop.clone();
                    let _ = gstreamer::glib::timeout_add_seconds_local(
        ...
        let receiver_handle = tokio::task::spawn_blocking(move || {
            let mut ns_mgr = NetNsManager::new().expect("Failed to create NetNsManager");
            ns_mgr
                .attach_existing_namespace("rx0_link_1")
                .expect("Failed to attach rx0_link_1");
        ...
        let assessment = assess_video_quality(
            &final_receiver_metrics,
            duration_secs,
            "Race Car Cellular Bonding with NetNS",
        );
    ...
    async fn run_video_only_over_netns(
        duration_secs: u64,
    ) -> Result<QualityAssessment, Box<dyn std::error::Error>> {
        info!(
            "Starting minimal video-only RIST test over netns for {}s",
            duration_secs
        );
        ...
        let (_orchestrator, bonding_addresses) = setup_race_car_network_scenarios(rx_port).await?;
        info!(
            "Netns setup complete for video-only test targeting {}",
            bonding_addresses.join(",")
        );
        ...
        let sender_handle = tokio::task::spawn_blocking(move || {
            ...
            let _ = ns_mgr.exec_in_namespace("tx0_link_1", || {
                gstreamer::init().ok();
                let sender_pipeline =
                    build_video_only_sender_pipeline(&bonding_addresses_tx).expect("build sender");
        ...
        while start_time.elapsed().as_secs() < duration_secs {
            ...
            if last_log.elapsed() >= interval {
                ...
                info!(
                    "Video-only progress: {}s - {} frames, {} KB",
                    start_time.elapsed().as_secs(),
                    m.frames_received,
    ...
    async fn test_race_car_rist_over_challenging_network() {
        ...
        match run_race_car_rist_test(test_duration).await {
            Ok(assessment) => {
                ...
                info!(
                    "Bonding Effectiveness: {:.1}%",
                    assessment.bonding_effectiveness
                );
                ...
                assert!(
                    ...
                    "Video quality score too low: {:.1}/100",
    ...
    async fn test_race_car_pipeline_smoke_test() {
        ...
        let bonding_addresses = vec!["127.0.0.1:9000".to_string(), "127.0.0.1:9002".to_string()];
        ...
        let (receiver_pipeline, _receiver_metrics) =
            build_race_car_receiver_pipeline(9100).expect("Failed to build receiver pipeline");
    ...
    async fn test_race_car_with_netns_and_video_output() {
        ...
        let test_duration = 15; // 15 seconds for testing
        ...
        match run_race_car_rist_test_with_video_output(test_duration).await {
            Ok(assessment) => {
                ...
                // Test assertions
                assert!(
                    assessment.overall_score >= 40.0,
                    "Video quality score too low for challenging conditions: {:.1}/100",
                    assessment.overall_score
                );
                assert!(metrics.frames_received > 0, "No frames received");
    ...
    async fn test_video_only_over_netns() {
        ...
        match run_video_only_over_netns(duration).await {
            Ok(assessment) => {
                ...
                assert!(
                    m.frames_received > 0,
                    "No frames received in video-only test"
                );
            }
            Err(e) => {
