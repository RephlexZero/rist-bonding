...
fn test_dispatcher_with_rist_elements() {
    ...
    pipeline.add_many([
        &source, &dispatcher, 
        &rist_sender1, &rist_sender2, 
        &rist_receiver1, &rist_receiver2
    ]).unwrap();
    ...
    src_0.link(&rist_sender1.static_pad("sink").unwrap()).unwrap();
    src_1.link(&rist_sender2.static_pad("sink").unwrap()).unwrap();
    ...
    assert!(balance_ratio >= 0.2, "Load balancing should be reasonably distributed");
...
fn test_dispatcher_with_dynamic_bitrate() {
    ...
    src_0.link(&dynbitrate1.static_pad("sink").unwrap()).unwrap();
    src_1.link(&dynbitrate2.static_pad("sink").unwrap()).unwrap();
    ...
    println!("Final counts: DynBitrate1={}, DynBitrate2={}", final_count1, final_count2);
    
    assert!(final_count1 >= count1, "DynBitrate 1 should continue processing");
    assert!(final_count2 >= count2, "DynBitrate 2 should continue processing");
...
fn test_dispatcher_with_stats_feedback() {
    ...
    println!("Initial counts: Counter1={}, Counter2={}", initial_count1, initial_count2);
    println!("Final counts: Counter1={}, Counter2={}", final_count1, final_count2);
    ...
    assert!(final_count1 >= initial_count1, "Counter 1 should continue processing");
    assert!(final_count2 >= initial_count2, "Counter 2 should continue processing");
...
fn test_complex_pipeline_integration() {
    ...
    pipeline.add_many([
        ...
        &dynbitrate1, &dynbitrate2, &dynbitrate3,
        &sink1, &sink2, &sink3
    ]).unwrap();
    ...
    println!("Path counts: Sink1={}, Sink2={}, Sink3={}", count1, count2, count3);
    ...
    println!("Final counts: Sink1={}, Sink2={}, Sink3={}", final_count1, final_count2, final_count3);
    ...
    assert!(total_count >= 100, "Complex pipeline should process significant data");
...
fn test_multi_dispatcher_coordination() {
    ...
    pipeline.add_many([
        ...
        &proc1_1, &proc1_2, &proc2_1, &proc2_2
    ]).unwrap();
    ...
    tee_src_0.link(&dispatcher1.static_pad("sink").unwrap()).unwrap();
    tee_src_1.link(&dispatcher2.static_pad("sink").unwrap()).unwrap();
    ...
    disp1_src_0.link(&proc1_1.static_pad("sink").unwrap()).unwrap();
    disp1_src_1.link(&proc1_2.static_pad("sink").unwrap()).unwrap();
    disp2_src_0.link(&proc2_1.static_pad("sink").unwrap()).unwrap();
    disp2_src_1.link(&proc2_2.static_pad("sink").unwrap()).unwrap();
    ...
    println!("Dispatcher 1 outputs: proc1_1={}, proc1_2={}", count1_1, count1_2);
    println!("Dispatcher 2 outputs: proc2_1={}, proc2_2={}", count2_1, count2_2);
    ...
    println!("  Dispatcher 1: proc1_1={}, proc1_2={}", final_count1_1, final_count1_2);
    println!("  Dispatcher 2: proc2_1={}, proc2_2={}", final_count2_1, final_count2_2);
    
    // Verify both dispatchers processed data
    assert!(final_count1_1 + final_count1_2 > 0, "Dispatcher 1 should process data");
    assert!(final_count2_1 + final_count2_2 > 0, "Dispatcher 2 should process data");
    
    let total_processed = final_count1_1 + final_count1_2 + final_count2_1 + final_count2_2;
    assert!(total_processed >= 100, "Multi-dispatcher setup should process significant data");
...
fn test_error_propagation_integration() {
    ...
    pipeline.add_many([&source, &dispatcher, &good_processor, &error_processor]).unwrap();
    ...
    src_0.link(&good_processor.static_pad("sink").unwrap()).unwrap();
    src_1.link(&error_processor.static_pad("sink").unwrap()).unwrap();
    ...
    println!("Good processor: {} -> {}", initial_good_count, final_good_count);
    println!("Error processor: {} -> {}", initial_error_count, final_error_count);
    ...
    assert!(initial_good_count > 0, "Good processor should process some data initially");
    assert!(initial_error_count > 0, "Error processor should process some data initially");
    
    // The important thing is that the dispatcher remained stable during the error simulation
    assert!(final_good_count >= initial_good_count, "Good processor should not lose data");
    assert!(final_error_count >= initial_error_count, "Error processor should not lose data");
