...
fn test_dispatcher_weight_switching_behavior() {
    ...
    if updates.is_empty() {
        ...
        assert!(updates.len() <= 8, "Should not update excessively due to hysteresis");
    }
...
fn test_dispatcher_warmup_period() {
    ...
    println!("Warmup test - early: {}, mid: {}, late: {}", 
             early_weights, mid_weights, late_weights);
    
    // During warmup, weights should be available but decision-making may be delayed
    assert!(!early_weights.is_empty(), "Should have weight information during warmup");
    assert!(!late_weights.is_empty(), "Should have weight information after warmup");
}
...
fn test_switch_threshold_enforcement() {
    ...
    println!("Threshold test - changes after small: {}, after large: {}", 
    ...
    assert!(changes_after_large >= changes_after_small, 
...
fn test_dispatcher_stats_during_hysteresis() {
    ...
    assert!(!initial_weights.is_empty(), "Should have current weights available");
    ...
    println!("Stats test - initial: {}, updated: {}", initial_weights, updated_weights);
}