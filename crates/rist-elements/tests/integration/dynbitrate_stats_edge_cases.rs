...
fn test_dynbitrate_configuration_validation() {
    ...
    for (min_kbps, max_kbps, step_kbps, target_loss, description) in &configs {
        ...
        assert_eq!(
            actual_min, *min_kbps,
            "min-kbps should match configured value"
        );
        assert_eq!(
            actual_max, *max_kbps,
            "max-kbps should match configured value"
        );
        assert_eq!(
            actual_step, *step_kbps,
            "step-kbps should match configured value"
        );
        assert!(
            (actual_target - target_loss).abs() < 0.001,
            "target-loss-pct should match configured value"
        );
