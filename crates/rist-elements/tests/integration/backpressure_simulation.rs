...
use std::sync::{Arc, Mutex, Barrier, atomic::{AtomicBool, AtomicU64, Ordering}};
...
fn test_slow_sink_backpressure() {
    ...
    pipeline.add_many([&src, &dispatcher, &sink1, &sink2]).unwrap();
    ...
    dispatcher.connect("notify::current-weights", false, move |values| {
        ...
        flow_counts_clone.lock().unwrap().push((weights_str, timestamp));
        None
    });
    ...
    println!("Backpressure test: {} flow changes, {} errors", 
             flow_history.len(), error_count);
    
    // Should handle backpressure gracefully
    assert_eq!(error_count, 0, "Should not generate errors during backpressure");
    
    // System should continue operating
    if flow_history.is_empty() {
        println!("No flow changes observed - system may have maintained stability");
    } else {
        assert!(flow_history.len() <= 10, "Should not have excessive flow changes during backpressure");
    }
}
...
fn test_blocked_output_handling() {
    ...
    dispatcher.connect("notify::current-weights", false, move |values| {
        ...
        let weights_str: String = elem.property("current-weights");
        let timestamp = Instant::now();
        switch_events_clone.lock().unwrap().push((weights_str, timestamp));
        None
    });
    ...
    println!("Testing blocked output handling with {} scenarios", blocking_scenarios.len());
    ...
    for (weights, description) in blocking_scenarios.iter() {
        ...
        assert!(!current.is_empty(), "Should maintain valid weights during blocking scenario");
    }
    ...
    if events.is_empty() {
        ...
        assert!(events.len() <= 15, "Should not have excessive switching during blocking scenarios");
    }
}
...
fn test_multiple_slow_outputs() {
    ...
    println!("Testing multiple slow outputs with {} degradation steps", 
...
fn test_backpressure_recovery() {
    ...
    dispatcher.connect("notify::current-weights", false, move |values| {
        ...
        recovery_phases_clone.lock().unwrap().push((elapsed, weights));
        None
    });
    ...
    let recovery_cycle = vec![
        ...
        ("[1.0, 0.3]", Duration::from_millis(500), "Output 1 under pressure"),
        ("[1.0, 0.1]", Duration::from_millis(400), "Severe backpressure"), 
        ("[1.0, 0.6]", Duration::from_millis(300), "Pressure reducing"),
        ("[1.0, 0.9]", Duration::from_millis(250), "Almost recovered"),
        ("[1.0, 1.2]", Duration::from_millis(300), "Full recovery + improvement"),
    ];
    
    println!("Testing backpressure recovery cycle with {} phases", recovery_cycle.len());
    
    for (phase_idx, (weights, duration, description)) in recovery_cycle.iter().enumerate() {
        dispatcher.set_property("weights", *weights);
        println!("Recovery phase {}: {} - {}", phase_idx + 1, description, weights);
    ...
    println!("Backpressure recovery: {} phases over {:?}", 
             phases.len(), total_recovery_time);
    ...
    assert!(!final_weights.is_empty(), "Should have valid weights after recovery");
}