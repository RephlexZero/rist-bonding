...
fn test_extended_auto_rebalancing_cycles() {
    ...
    println!(
        "Starting extended auto-rebalancing test with {} cycles",
        condition_cycles.len()
    );
    ...
    for (cycle_idx, (weights, duration)) in condition_cycles.iter().enumerate() {
        ...
        assert!(
            ...
            "Should maintain valid weights during cycle {}",
            cycle_idx + 1
        );
    }
    ...
    println!(
        "Extended auto-rebalancing: {} weight changes over {:?}",
        history.len(),
        total_duration
    );
    ...
    if history.is_empty() {
        ...
        assert!(
            history.len() <= 20,
            "Should not have excessive weight changes"
        );
        ...
        for i in 1..history.len() {
            ...
            assert!(
                time_gap >= Duration::from_millis(50), // Not too rapid
                "Weight changes should not be too rapid"
            );
            assert!(
                time_gap <= Duration::from_millis(1500), // Not too slow
...
fn test_weight_adaptation_over_time() {
    ...
    if snapshots.is_empty() {
        ...
        assert!(
            snapshots.len() <= 25,
            "Should not have excessive weight snapshots"
        );
        ...
        if unique_weights.len() > 1 {
            println!(
                "Observed {} different weight states during adaptation",
                unique_weights.len()
            );
        }
    }
...
fn test_rapid_oscillation_handling() {
    ...
    *oscillation_count.lock().unwrap() = 0;
    ...
    println!(
        "Testing rapid oscillation handling with {} pattern changes",
    ...
    println!(
        "Rapid oscillation test: {} weight change notifications",
        total_oscillations
    );

    // Should handle oscillations without excessive thrashing
    assert!(
        total_oscillations <= 15,
        "Should limit thrashing during rapid oscillations: got {}",
        total_oscillations
    );
    ...
    assert!(
        ...
        "Should maintain valid final state"
    );
}
...
fn test_long_term_rebalancing_stability() {
    ...
    dispatcher.connect("notify::current-weights", false, move |values| {
        ...
        stability_metrics_clone
            .lock()
            .unwrap()
            .push((elapsed, weights_str));
        None
    });
    ...
    println!(
        "Testing long-term stability over {} condition changes",
    ...
    for (phase, (condition, duration)) in long_term_conditions.iter().enumerate() {
        ...
        assert!(
            !current.is_empty(),
            "Should maintain stability in phase {}",
            phase + 1
        );
    }
    ...
    println!(
        "Long-term stability: {} measurements over {:?}",
        metrics.len(),
        total_test_time
    );
    ...
    if metrics.is_empty() {
        ...
        if metrics.len() >= 2 {
            ...
            assert!(
                time_span >= Duration::from_millis(1000),
