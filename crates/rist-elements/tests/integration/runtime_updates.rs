...
fn test_runtime_weight_updates() {
    ...
    for weights in weight_sequences.iter() {
        ...
        assert!(
            current_weights.contains(&normalized_weights[1..normalized_weights.len() - 1])
                || current_weights == normalized_weights,
            "Weights should be updated during runtime: expected {}, got {}",
            weights,
            current_weights
        );
    }
    ...
    println!(
        "Runtime weight test: {} changes over {:?}",
        changes.len(),
        test_duration
    );
    ...
    if changes.is_empty() {
        ...
        assert!(
            ...
            "Should maintain valid current weights"
        );
    } else {
        assert!(
            changes.len() <= 15,
            "Should not have excessive change notifications"
        );
        ...
        for i in 1..changes.len() {
            ...
            assert!(
                time_gap <= Duration::from_millis(2000),
...
fn test_runtime_strategy_changes() {
    ...
    println!(
        "Strategy test - manual: {}, auto: {}, final: {}",
        manual_weights, auto_weights, final_manual_weights
    );

    // All should have valid weight strings
    assert!(
        !manual_weights.is_empty(),
        "Manual strategy should produce weights"
    );
    assert!(
        !auto_weights.is_empty(),
        "Auto strategy should produce weights"
    );
    assert!(
        !final_manual_weights.is_empty(),
        "Final manual strategy should produce weights"
    );
    ...
    assert!(
        ["manual", "auto", "ewma"].contains(&current_strategy.as_str()),
        "Strategy should be in a valid state: {}",
        current_strategy
    );
}
...
fn test_concurrent_weight_updates() {
    ...
    println!(
        "Concurrent test: {} successful updates, {} errors",
        final_updates, final_errors
    );

    // Should handle most updates successfully
    assert!(
        final_updates >= 10,
        "Should handle most concurrent weight updates"
    );
    assert!(
        final_errors <= 5,
        "Should have minimal errors from concurrent updates"
    );
    ...
    assert!(
        !final_weights.is_empty(),
        "Should maintain valid weights after concurrent updates"
    );
}
...
fn test_runtime_property_validation() {
    ...
    assert_eq!(
        min_hold, 300,
        "Min hold time should be updatable during runtime"
    );
    assert_eq!(
        threshold, 1.8,
        "Switch threshold should be updatable during runtime"
    );
    assert_eq!(
        interval, 500,
        "Rebalance interval should be updatable during runtime"
    );
...
fn test_runtime_weight_format_validation() {
    ...
    for weights in valid_formats {
        ...
        assert!(
            !current.is_empty(),
            "Should accept valid weight format: {}",
            weights
        );
    }
