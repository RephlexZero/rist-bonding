...
fn test_downscale_keyunit_property() {
    ...
    assert!(
        !default_keyunit,
        "downscale-keyunit should default to false"
    );
    ...
    assert!(
        enabled_keyunit,
        "downscale-keyunit should be enabled after setting to true"
    );
    ...
    assert!(
        !disabled_keyunit,
        "downscale-keyunit should be disabled after setting to false"
    );
...
fn test_keyunit_configuration_scenarios() {
    ...
    let scenarios = [(true, "enabled"), (false, "disabled")];
    ...
    for (keyunit_enabled, description) in &scenarios {
        ...
        assert_eq!(
            actual_keyunit, *keyunit_enabled,
...
fn test_dynbitrate_with_keyunit_enabled() {
    ...
    pipeline
        .add_many([&dynbitrate, &encoder_stub, &riststats_mock])
        .unwrap();
