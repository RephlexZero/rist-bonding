...
fn test_high_throughput_performance() {
    ...
    println!("Processed {} buffers in {:.3}s", processed_count, elapsed.as_secs_f64());
    ...
    assert!(processed_count >= 1000, "Should process significant number of buffers");
...
fn test_dynamic_rebalancing_performance() {
    ...
    println!("Processed {} buffers with {} rebalances in {:.3}s", 
        final_count, rebalances, elapsed.as_secs_f64());
    
    let count1: u64 = get_property(&counter1, "count").unwrap_or(0);
    let count2: u64 = get_property(&counter2, "count").unwrap_or(0);
    println!("Final distribution: Counter1={}, Counter2={}", count1, count2);
    
    assert!(final_count >= 1000, "Should process significant buffers despite rebalancing");
...
fn test_memory_usage_under_load() {
    ...
    println!("Processed {} buffers in {:.3}s", max_total_count, elapsed.as_secs_f64());
    ...
    assert!(max_total_count >= 2000, "Should process significant number of large buffers");
...
fn test_concurrent_property_access_performance() {
    ...
    println!("Completed {} property updates and {} reads in {:.3}s", 
        updates, reads, elapsed.as_secs_f64());
    
    let update_rate = updates as f64 / elapsed.as_secs_f64();
    let read_rate = reads as f64 / elapsed.as_secs_f64();
    
    println!("Update rate: {:.2} ops/sec, Read rate: {:.2} ops/sec", update_rate, read_rate);
    
    assert!(updates >= 500, "Should complete significant number of updates");
    assert!(reads >= 1000, "Should complete significant number of reads");
    assert!(update_rate >= 50.0, "Should achieve reasonable update performance");
    assert!(read_rate >= 100.0, "Should achieve reasonable read performance");
...
fn test_pad_lifecycle_performance() {
    ...
    let elapsed = start_time.elapsed();
    println!("Created {} pads and destroyed {} pads in {:.3}s", 
        creation_count, destruction_count, elapsed.as_secs_f64());
    ...
    println!("Creation rate: {:.2} pads/sec, Destruction rate: {:.2} pads/sec", 
        creation_rate, destruction_rate);
    
    assert_eq!(creation_count, destruction_count, "Should create and destroy equal numbers");
    assert!(creation_count >= 200, "Should create significant number of pads");
    assert!(creation_rate >= 100.0, "Should achieve reasonable creation performance");
    assert!(destruction_rate >= 100.0, "Should achieve reasonable destruction performance");
...
fn test_algorithmic_performance_comparison() {
    ...
    for strategy in &strategies {
        ...
        println!("{} strategy: {} buffers in {:.3}s ({:.2} buf/sec)", 
            strategy, final_count, elapsed.as_secs_f64(), throughput);
    }
    ...
    for (strategy, count, _, throughput) in &results {
        assert!(*count >= 1000, "{} should process significant buffers", strategy);
        assert!(*throughput >= 50.0, "{} should achieve reasonable throughput", strategy);
    }
