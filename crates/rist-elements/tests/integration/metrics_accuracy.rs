...
fn test_property_accuracy_validation() {
    ...
    assert_eq!(interval, 250, "Metrics export interval should be accurately set");
    ...
    assert!(dup_keyframes, "Duplicate keyframes should accept boolean values");
...
fn test_metrics_export_interval_timing() {
    ...
    if !collected_messages.is_empty() {
        ...
        assert!(collected_messages.len() >= 1, "Should receive at least one metrics message");
    } else {
...
fn test_bus_message_accuracy() {
    ...
    if !collected_messages.is_empty() {
        ...
        for structure in collected_messages.iter() {
            ...
            assert!(structure.has_field("timestamp"), "Should contain timestamp field");
            assert!(structure.has_field("active-outputs"), "Should contain active-outputs field");
            assert!(structure.has_field("switch-count"), "Should contain switch-count field");
...
fn test_rapid_property_changes() {
    ...
    for i in 0..10 {
        ...
        assert!((actual_threshold - threshold).abs() < 0.01, "Threshold should update correctly: expected {}, got {}", threshold, actual_threshold);
    }
...
fn test_rebalance_interval_validation() {
    ...
    for interval in test_intervals {
        ...
        assert_eq!(retrieved_interval, interval, "Rebalance interval should be set correctly");
