...
fn test_latency_variation() {
    ...
    assert!(
        ...
        "Weights should adapt to simulated conditions"
    );
}
...
fn test_packet_loss_adaptation() {
    ...
    assert_ne!(
        final_weights, initial_weights,
        "Weights should adapt to packet loss simulation"
    );
}
...
fn test_bandwidth_adaptation() {
    ...
    assert_ne!(
        final_weights, initial_weights,
        "Should adapt to bandwidth simulation"
    );
}
...
fn test_intermittent_connectivity() {
    ...
    for (cycle, pattern) in connectivity_patterns.iter().enumerate() {
        ...
        events_clone
            .lock()
            .unwrap()
            .push(format!("Cycle {}: weights {}", cycle, pattern));
        ...
        assert!(
            !weights.is_empty(),
            "Should maintain valid weights during connectivity changes"
        );
    }
    ...
    assert!(
        !events.is_empty(),
        "Should have recorded connectivity events"
    );
}
...
fn test_congestion_bursts() {
    ...
    assert!(
        adaptation_count > 0,
        "Should have adapted to congestion simulation"
    );
}
...
fn test_mixed_network_conditions() {
    ...
    assert!(
        ...
        "Test should complete efficiently"
    );
    assert!(
        condition_changes > 0,
