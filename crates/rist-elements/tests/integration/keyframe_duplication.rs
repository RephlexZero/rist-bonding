...
fn test_keyframe_duplication_properties() {
    ...
    assert!(
        !default_dup_keyframes,
        "Default duplicate-keyframes should be false"
    );
    ...
    assert!(
        enabled_dup_keyframes,
        "duplicate-keyframes should be settable to true"
    );
    assert_eq!(
        new_dup_budget, 10,
        "dup-budget-pps should be settable to 10"
    );
    ...
    assert_eq!(
        min_budget, 0,
        "dup-budget-pps should accept minimum value 0"
    );
    ...
    assert_eq!(
        max_budget, 100,
        "dup-budget-pps should accept maximum value 100"
    );
...
fn test_keyframe_duplication_disabled_by_default() {
    ...
    assert!(
        !dup_enabled,
        "Keyframe duplication should be disabled by default"
    );
...
fn test_keyframe_duplication_budget_limits() {
    ...
    println!(
        "  After first phase - Path 1: {}, Path 2: {}",
        count1_low, count2_low
    );
    println!(
        "  After second phase - Path 1: {}, Path 2: {}",
        count1_high, count2_high
    );
    ...
    assert!(
        ...
        "Pipeline should continue producing traffic (first phase: {}, second phase: {})",
        total_low,
        total_high
    );
...
fn test_keyframe_duplication_with_pad_switching() {
    ...
    assert!(
        total_traffic > 0,
        "Should receive traffic with keyframe duplication enabled"
    );
    ...
    if count1_final > 0 && count2_final > 0 {
        ...
        println!(
            "Traffic concentrated on primary path with occasional duplication (expected behavior)"
        );
    }
    ...
    if delta2 > delta1 {
        println!(
            "Weight switching successfully affected traffic distribution during duplication phase"
        );
    }
...
fn test_keyframe_duplication_budget_reset() {
    ...
    assert!(
        dup_enabled,
        "Keyframe duplication should be enabled for budget reset test"
    );
    ...
    println!(
        "  After potential reset: {} total buffers",
        count_after_reset
    );
    ...
    assert!(
        count_after_reset >= count_burst,
        "Should have consistent traffic over time (burst: {}, after: {})",
        count_burst,
        count_after_reset
    );
