...
async fn test_scenario_builder() {
    ...
    assert_eq!(scenario.metadata.get("test_type"), Some(&"integration".to_string()));
    assert_eq!(scenario.metadata.get("environment"), Some(&"ci".to_string()));
...
async fn test_crate_imports() {
    ...
    match result {
        ...
        Err(e) => {
            println!("⚠️  NetworkOrchestrator creation failed (likely permissions): {}", e);
        }
    }
