...
impl QdiscManager {
    ...
    pub async fn add_netem(&self, _handle: &Handle, interface_index: u32, config: NetemConfig) -> Result<(), QdiscError> {
        debug!("Adding netem qdisc to interface {}: {:?}", interface_index, config);
        ...
        info!("Netem qdisc configured for interface {} (rate: {}bps, delay: {}us, loss: {}%)", 
              interface_index, config.rate_bps, config.delay_us, config.loss_percent);
        Ok(())
    }

    /// Add TBF qdisc to interface
    pub async fn add_tbf(&self, _handle: &Handle, interface_index: u32, config: TbfConfig) -> Result<(), QdiscError> {
        debug!("Adding TBF qdisc to interface {}: {:?}", interface_index, config);
        
        info!("TBF qdisc configured for interface {} (rate: {}bps, burst: {}bytes)", 
              interface_index, config.rate_bps, config.burst_bytes);
        Ok(())
    }

    /// Update netem parameters
    pub async fn update_netem(&self, handle: &Handle, interface_index: u32, config: NetemConfig) -> Result<(), QdiscError> {
        debug!("Updating netem qdisc on interface {}: {:?}", interface_index, config);
    ...
    pub async fn remove_qdisc(&self, _handle: &Handle, interface_index: u32) -> Result<(), QdiscError> {
    ...
    pub async fn apply_direction_spec(&self, handle: &Handle, interface_index: u32, spec: &scenarios::DirectionSpec) -> Result<(), QdiscError> {
        debug!("Applying DirectionSpec to interface {}: {:?}", interface_index, spec);
    ...
    pub async fn list_qdiscs(&self, _handle: &Handle, interface_index: u32) -> Result<Vec<String>, QdiscError> {
        debug!("Listing qdiscs on interface {}", interface_index);
        
        // Return empty list for now - full implementation would query via netlink
        let qdiscs = Vec::new();
        
        debug!("Found {} qdiscs on interface {}", qdiscs.len(), interface_index);
        Ok(qdiscs)
    }
    ...
    #[allow(dead_code)]
    async fn tc_command_example(&self, interface_name: &str, config: &NetemConfig) -> Result<(), QdiscError> {
        use tokio::process::Command;
        
        // Example: tc qdisc add dev veth0 root netem delay 100ms 10ms loss 1% rate 1mbit
        let mut cmd = Command::new("tc");
        cmd.args(&[
            "qdisc", "add", "dev", interface_name, "root", "netem"
        ]);
        ...
        let output = cmd.output().await.map_err(|e| QdiscError::MessageEncoding(e.to_string()))?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(QdiscError::MessageEncoding(format!("tc command failed: {}", stderr)));
        }
