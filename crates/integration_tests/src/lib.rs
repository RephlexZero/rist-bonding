...
use tokio::{time::sleep, net::UdpSocket, process::Command};
...
impl RistIntegrationTest {
    ...
    pub async fn new(test_id: String, rx_port: u16) -> Result<Self> {
        ...
        let orchestrator = netlink_sim::enhanced::EnhancedNetworkOrchestrator::new_with_observability(
            42, Some(&trace_path)
        ).await?;
    ...
    pub async fn start_rist_dispatcher(&mut self) -> Result<()> {
        ...
        cmd.args(&["run", "--bin", "ristdispatcher", "--"])
            .args(&[
                "--rx-port", &self.rx_port.to_string(),
                "--stats-interval", "1000",
                "--adaptive-bitrate", "true",
                "--bonding-mode", "main-backup",
                "--max-bitrate", "2000", // Race car realistic max
                "--min-bitrate", "300",  // Race car realistic min
            ])
    ...
    pub async fn setup_race_car_bonding(&mut self) -> Result<Vec<netlink_sim::LinkHandle>> {
        ...
        let links = self.orchestrator.start_race_car_bonding(self.rx_port).await?;
    ...
    pub async fn run_race_car_test_pattern(&mut self) -> Result<TestResults> {
        ...
        self.simulate_traffic_phase("strong", Duration::from_secs(10)).await?;
        ...
        self.simulate_traffic_phase("degraded", Duration::from_secs(15)).await?;
        ...
        self.simulate_traffic_phase("handover", Duration::from_secs(8)).await?;
        ...
        self.simulate_traffic_phase("recovery", Duration::from_secs(12)).await?;
        results.add_phase("recovery", self.collect_phase_metrics().await?);

        results.total_duration = start_time.elapsed();
        println!("✓ Race car test pattern completed ({:.1}s)\n", 
    ...
    pub async fn validate_bonding_behavior(&self, results: &TestResults) -> Result<ValidationReport> {
        println!("🔍 Validating RIST bonding behavior...");
        
        let mut report = ValidationReport::new();

        // Check adaptive bitrate behavior
        let bitrate_adapted = results.phases.iter()
            .any(|(phase, metrics)| {
                if phase == "degraded" {
        ...
        let bonding_effective = results.phases.iter()
        ...
        let balanced_utilization = results.phases.iter()
            .all(|(_, metrics)| {
        ...
        println!("  - Adaptive bitrate: {}", if report.adaptive_bitrate_working { "✅" } else { "❌" });  
        println!("  - Bonding effectiveness: {}", if report.bonding_effective { "✅" } else { "❌" });
        println!("  - Load balancing: {}", if report.load_balancing_working { "✅" } else { "❌" });
        println!();
    ...
    async fn build_rist_dispatcher(&self) -> Result<()> {
        ...
        if !output.status.success() {
            return Err(anyhow::anyhow!("Failed to build RIST dispatcher: {}", 
    ...
    async fn apply_degradation_schedule(&mut self) -> Result<()> {
        ...
        self.orchestrator.apply_schedule("race_4g_primary", degraded_schedule.clone()).await?;
        self.orchestrator.apply_schedule("race_5g_primary", degraded_schedule).await?;
        Ok(())
    }

    async fn trigger_handover_event(&mut self) -> Result<()> {
        let handover_schedule = scenarios::Schedule::race_4g_markov(); // High variability
        self.orchestrator.apply_schedule("race_4g_primary", handover_schedule).await?;
        self.orchestrator.apply_schedule("race_4g_backup", scenarios::Schedule::race_5g_markov()).await?;
        Ok(())
    }
    ...
    async fn apply_recovery_schedule(&mut self) -> Result<()> {
        ...
        self.orchestrator.apply_schedule("race_5g_primary", recovery_schedule).await?;
        Ok(())
    }
    ...
    async fn collect_phase_metrics(&self) -> Result<PhaseMetrics> {
        if let Some(snapshot) = self.orchestrator.get_metrics_snapshot().await? {
            Ok(PhaseMetrics {
                avg_bitrate: snapshot.link_performance.iter()
                    .map(|m| m.link_stats.throughput_bps / 1000)
                    .sum::<u64>() as f64 / snapshot.link_performance.len().max(1) as f64,
                packet_loss: snapshot.link_performance.iter()
                    .map(|m| m.link_stats.loss_rate)
                    .sum::<f64>() / snapshot.link_performance.len().max(1) as f64,
                avg_rtt: snapshot.link_performance.iter()
