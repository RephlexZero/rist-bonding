    ...
    async fn test_docker_network_environment_setup() {
        ...
        match env.setup_basic_network().await {
            Ok(()) => {
                ...
                let result = env.apply_network_impairments("veth_test0", 10, 0.5, 5000).await;
                match result {
                ...
                match connectivity {
                    ...
                    Ok(false) => println!("ℹ️ Connectivity test failed (may be expected in some environments)"),
                    Err(e) => println!("⚠️ Connectivity test error: {}", e),
                }
            }
            Err(DockerNetworkError::CommandFailed(msg)) if msg.contains("Operation not permitted") => {
                println!("ℹ️ Network setup requires proper Docker capabilities (--cap-add=NET_ADMIN)");
            }
            Err(e) => {
    ...
    async fn test_network_impairment_scenarios() {
        ...
        if env.setup_basic_network().await.is_ok() {
            ...
            for (name, delay_ms, loss_pct, rate_kbps) in scenarios {
                ...
                let result = env.apply_network_impairments("veth_test0", delay_ms, loss_pct, rate_kbps).await;
                match result {
                    Ok(()) => println!("  ✅ Applied: {}ms delay, {}% loss, {} kbps", delay_ms, loss_pct, rate_kbps),
...
mod basic_network_sim_tests {
    use network_sim::{NetworkParams, apply_network_params};
