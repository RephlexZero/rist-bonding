...
impl DockerNetworkEnv {
    ...
    pub async fn setup_basic_network(&mut self) -> Result<(), DockerNetworkError> {
        ...
        self.move_interface_to_namespace("veth_test1", "test_ns1")
            .await?;
        self.move_interface_to_namespace("veth_test3", "test_ns2")
            .await?;

        // Configure IP addresses
        self.configure_interface("veth_test0", "192.168.100.1/24")
            .await?;
        self.configure_interface("veth_test2", "192.168.101.1/24")
            .await?;

        self.configure_interface_in_namespace("test_ns1", "veth_test1", "192.168.100.2/24")
            .await?;
        self.configure_interface_in_namespace("test_ns2", "veth_test3", "192.168.101.2/24")
            .await?;
        ...
        self.bring_up_interface_in_namespace("test_ns1", "veth_test1")
            .await?;
        self.bring_up_interface_in_namespace("test_ns2", "veth_test3")
            .await?;
    ...
    pub async fn configure_interface(
        ...
        if !output.status.success() {
            ...
            if !stderr.contains("File exists") {
                return Err(DockerNetworkError::InterfaceError(format!(
                    "Failed to configure interface {} with {}: {}",
                    interface, addr, stderr
                )));
            }
        }
    ...
    pub async fn configure_interface_in_namespace(
        &self,
        namespace: &str,
        interface: &str,
        addr: &str,
        ...
        let output = Command::new("ip")
            ...
            .map_err(|e| {
                DockerNetworkError::CommandFailed(format!(
                    "Failed to configure interface in namespace: {}",
                    e
                ))
            })?;
        ...
        if !output.status.success() {
            ...
            if !stderr.contains("File exists") {
                return Err(DockerNetworkError::InterfaceError(format!(
                    "Failed to configure interface {} in namespace {} with {}: {}",
                    interface, namespace, addr, stderr
                )));
            }
        }
    ...
    pub async fn bring_up_interface_in_namespace(
        &self,
        namespace: &str,
        interface: &str,
    ) -> Result<(), DockerNetworkError> {
    ...
    pub async fn test_connectivity(
        ...
        let output = Command::new("ip")
            ...
            .output()
            .map_err(|e| {
                DockerNetworkError::CommandFailed(format!("Failed to test connectivity: {}", e))
            })?;
    ...
    pub async fn apply_network_impairments(
        &self,
        interface: &str,
        delay_ms: u32,
        loss_pct: f32,
        ...
        let output = Command::new("tc")
            ...
            .map_err(|e| {
                DockerNetworkError::CommandFailed(format!(
                    "Failed to apply network impairments: {}",
                    e
                ))
            })?;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(DockerNetworkError::InterfaceError(format!(
                "Failed to apply network impairments to {}: {}",
                interface, stderr
            )));
        }
        ...
        match env.setup_basic_network().await {
            Ok(()) => {
                ...
                match env.test_connectivity("test_ns1", "192.168.100.1").await {
                    ...
                    Ok(false) => {
                        println!("⚠ Connectivity test failed (expected in some environments)")
                    }
                ...
                match env
                    .apply_network_impairments("veth_test0", 50, 1.0, 1000)
                    .await
                {
            ...
            Err(e) => {
                println!(
                    "⚠ Network setup failed (expected without proper privileges): {}",
                    e
                );
            }
        }
