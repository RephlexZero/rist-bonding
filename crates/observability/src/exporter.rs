...
impl PrometheusExporter {
    ...
    fn format_prometheus_line(&self, name: &str, value: f64, labels: &[(&str, &str)], help: &str) -> String {
        ...
        for link in &snapshot.link_performance {
            ...
            if let Some(delay) = link.qdisc_params.delay_ms {
                output.push_str(&self.format_prometheus_line(
                    ...
                    delay as f64,
                    &[("link_id", link_id), ("qdisc", &link.qdisc_params.qdisc_type)],
    ...
    async fn export(&self, snapshot: &MetricsSnapshot) -> Result<String> {
        ...
        for link in &snapshot.link_performance {
            let line = format!(
                ...
                link.qdisc_params.delay_ms.map_or("".to_string(), |d| d.to_string()),
                link.qdisc_params.rate_kbps.map_or("".to_string(), |r| r.to_string()),
                link.qdisc_params.loss_pct.map_or("".to_string(), |l| l.to_string()),
            );
