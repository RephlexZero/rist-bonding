...
use gstristsmart::{test_pipeline, link_elements};
...
fn test_integrated_dispatcher_flow() {
    ...
    src_0.link(&counter1.static_pad("sink").unwrap()).expect("Failed to link dispatcher src_0");
    src_1.link(&counter2.static_pad("sink").unwrap()).expect("Failed to link dispatcher src_1");
    ...
    println!("Counter 1: {} buffers, Counter 2: {} buffers", count1, count2);
...
fn test_stats_driven_rebalancing() {
    ...
    let retrans_0 = recovered_stats.get::<u64>("session-0.sent-retransmitted-packets").unwrap();
    let rtt_0 = recovered_stats.get::<f64>("session-0.round-trip-time").unwrap();
    
    assert!(retrans_0 < 55, "Retransmissions should be reduced after recovery");
...
fn test_dynbitrate_integration() {
    ...
    let initial_bitrate: u32 = get_property(&encoder, "bitrate")
        .expect("Failed to get encoder bitrate");
