...
fn test_weighted_flow_distribution() {
    ...
    let src_pad3 = dispatcher
        .request_pad_simple("src_%u")
        .expect("Failed to request pad 3");
    ...
    println!(
        "  Counter 1: {} buffers ({:.1}%)",
        count1,
        count1 as f64 / total as f64 * 100.0
    );
    println!(
        "  Counter 2: {} buffers ({:.1}%)",
        count2,
        count2 as f64 / total as f64 * 100.0
    );
    println!(
        "  Counter 3: {} buffers ({:.1}%)",
        count3,
        count3 as f64 / total as f64 * 100.0
    );
    ...
    assert!(
        (ratio1 - 0.5).abs() < 0.1,
        "Counter1 should receive ~50% of buffers, got {:.1}%",
        ratio1 * 100.0
    );
    assert!(
        (ratio2 - 0.3333).abs() < 0.1,
        "Counter2 should receive ~33% of buffers, got {:.1}%",
        ratio2 * 100.0
    );
    assert!(
        (ratio3 - 0.1667).abs() < 0.1,
        "Counter3 should receive ~17% of buffers, got {:.1}%",
        ratio3 * 100.0
    );
...
fn test_dynamic_weight_adjustment() {
    ...
    println!(
        "Phase 1 (equal weights): Counter1={}, Counter2={}",
    ...
    println!(
        "Phase 2 (adjusted weights [1.0, 4.0]): Counter1=+{}, Counter2=+{}",
        delta1, delta2
    );
    ...
    if delta1 + delta2 > 10 {
        ...
        if delta1 > 0 {
            ...
            assert!(
                ratio > 1.5,
                "Weight adjustment should significantly affect traffic distribution"
            );
        }
    }
...
fn test_extreme_weight_ratios() {
    ...
    println!(
        "  Counter 1 (high weight): {} buffers ({:.1}%)",
        count1,
        count1 as f64 / total as f64 * 100.0
    );
    println!(
        "  Counter 2 (low weight): {} buffers ({:.1}%)",
        count2,
        count2 as f64 / total as f64 * 100.0
    );
    ...
    println!(
        "Traffic distribution: {:.1}% to high weight, {:.1}% to low weight",
        ratio1 * 100.0,
        (1.0 - ratio1) * 100.0
    );
...
fn test_zero_weight_handling() {
    ...
    assert!(
        count1 > 0 || count3 > 0,
        "Non-zero weight counters should receive traffic"
    );
...
fn test_weight_normalization() {
    ...
    assert!(
        ...
        "Should be able to read back weights"
    );
