...
// Implement the testing provider trait for ristsmart
struct RistSmartTestingProvider;

impl DispatcherTestingProvider for RistSmartTestingProvider {
    ...
    fn wait_for_state_change(pipeline: &gst::Pipeline, state: gst::State, timeout_secs: u32) -> Result<(), gst::StateChangeError> {
        wait_for_state_change(pipeline, state, timeout_secs as u64).map_err(|_| gst::StateChangeError)
    }
    ...
    fn run_pipeline_for_duration(pipeline: &gst::Pipeline, duration_secs: u32) -> Result<(), Box<dyn std::error::Error>> {
        run_pipeline_for_duration(pipeline, duration_secs as u64)
    }
}

#[test]
fn test_caps_negotiation_and_proxying() {
    test_caps_negotiation_and_proxying::<RistSmartTestingProvider>();
}

#[test]
fn test_eos_event_fanout() {
    test_eos_event_fanout::<RistSmartTestingProvider>();
}

#[test]
fn test_flush_event_handling() {
    test_flush_event_handling::<RistSmartTestingProvider>();
}

#[test]
fn test_sticky_events_replay() {
    test_sticky_events_replay::<RistSmartTestingProvider>();
}

#[test]
fn test_pad_removal_and_cleanup() {
    test_pad_removal_and_cleanup::<RistSmartTestingProvider>();
}
