...
fn test_basic_data_flow_without_rist() {
    ...
    println!("Traffic distribution ratio: {:.2}", count1 as f64 / count2 as f64);
...
fn test_bonding_configuration() {
    ...
    println!("Bonding configuration - Interval: {}ms, Strategy: {}, Auto-balance: {}", 
...
fn test_rist_pipeline_with_transport() {
    ...
    test_pipeline!(pipeline, &source, &dispatcher, &ristsink, &ristsrc, &counter);
    ...
    src_0.link(&ristsink.static_pad("sink").unwrap()).expect("Failed to link to ristsink");
...
fn test_audio_pipeline_without_rist_transport() {
    ...
    test_pipeline!(pipeline, &source, &encoder, &dynbitrate, &dispatcher, 
                  &counter1, &counter2);
    ...
    assert!(count1 > 0 && count2 > 0, "Both outputs should receive audio data");
